...
const Busqueda = () => {
    ...
    //lo que hace este codigo es que revisa si busqueda esta vacio lo asigna a un array vacio o si no crea un array donde usa .filter para buscar en los proyectos tambien usa tolowecase para que se haga mas exacta la busqueda en ambos casos
                        ...
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {proyectosFiltrados.map(proyecto =>(
                                <Combobox.Option
                                    key={proyecto._id}
                                    value={proyecto}
                                    className={({active}) =>classNames('cursos-default select-none px-4 py-2',active && 'bg-sky-600 text-white')}
                                >{proyecto.nombre}</Combobox.Option>
                            ))}
                        </Combobox.Options>
                    )} 
                    </Combobox>
